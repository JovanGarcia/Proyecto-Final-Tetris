package {
	...
	public class Principal2 extends MovieClip {
		...
		var points2: Number;
		var tim2: String;
		//variable localconnection para tener comunicacion local con otro programa
		var connection: LocalConnection = new LocalConnection();

		var colores: Array = new Array();


		//FUNCION esperar: Espera al servidor a que inicie el y designe la dificultad del juego
		public function esperar(dificultad: String): void {
			trace(dificultad);
			dificultadnum = Number(dificultad);
			jugar();
		...
		public function Principal2() {
			//coneccion con el servidor para recibir los datos necesarios
		...
		function fplay(e: MouseEvent): void {
			...
			//stage.addEventListener(Event.ENTER_FRAME,esperar);
		}
			...
			function ft4(event: MouseEvent): void {
				if (insertname_txt.text == "") {
					...
					if (loss != 1) {
						...
						tiempoj = time.text + "";
						connection.send("cliente", "recibirdatos", nombre, puntos, tiempoj);
						gotoAndStop(4);
						if (loss > puntos) {
							names_txt.text = nam2 + "\n" + nombre + "\n";
						...
						nombre = insertname_txt.text + "";
						tiempoj = time.text + "";
						connection.send("cliente", "recibirdatos2", nombre, puntos, tiempoj);
						/*timer.stop();
		...
		public function recibirdatos1(nam, points, tim): void {
			loss = points;
			nam2 = nam;
			tim2 = tim;
		}
